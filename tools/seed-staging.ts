...
async function createMissing(kind: 'posts' | 'pages', fixtures: Array<{ title: string; content: string; labels?: string[] }>, existing: SeedResource[]): Promise<void> { const existingTitles = new Set(existing.map((item) => item.title)); for (const fixture of fixtures) { if (existingTitles.has(fixture.title)) { console.log(`skipped existing ${kind.slice(0, -1)}: ${fixture.title}`); continue; } await client.request(`${API}/blogs/${blogId}/${kind}`, { method: 'POST', body: JSON.stringify(fixture) }); console.log(`created ${kind.slice(0, -1)}: ${fixture.title}`); } }
await createMissing('posts', posts, existingPosts); await createMissing('pages', pages, existingPages);
