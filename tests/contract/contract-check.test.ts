...
const cases: readonly RuleCase[] = [
  ...
  { id: 'section-ids', violate: (xml) => inject(xml, '<b:section class="sidebar" id="invalid-hyphen" name="Sidebar" showaddelement="no"/>'), comment: 'section ids must use underscores rather than hyphens' },
  { id: 'no-maxwidgets', violate: (xml) => xml.replace('id="header" name="Header"', 'id="header" maxwidgets="1" name="Header"'), comment: 'maxwidgets blocks Blogger widget instantiation on custom section ids' },
  { id: 'header-widget', violate: (xml) => xml.replaceAll('type="Header"', 'type="HTML"'), comment: 'missing Header widget' },
  ...
  { id: 'no-fabricated-metadata', violate: (xml) => inject(xml, '<h3 class="sidebar-name">Fake Author</h3>'), comment: 'hardcoded author identity in sidebar is fabricated' },
  { id: 'robots-directives', violate: (xml) => xml.replace(/<meta content="noindex, follow" name="robots"\/>|<meta name="robots" content="noindex, follow"\/>/, ''), comment: 'robots meta directives must be declared in head' },
  ...
  { id: 'size-budget', violate: (xml) => `${xml.slice(0, -1)}<!--${'x'.repeat(500_001)}-->\n`, comment: 'output over 500000 bytes is invalid' },
  ...
  { id: 'rel-canonical', violate: (xml) => xml.replace('</head>', '<link rel="canonical" href="https://example.com"/></head>'), comment: 'manual canonical link creating duplicates is forbidden' },
  { id: 'opengraph-metadata', violate: (xml) => xml.replace('<meta property="og:site_name" expr:content="data:blog.title.escaped"/>', ''), comment: 'og:site_name metadata is mandatory in head' },
