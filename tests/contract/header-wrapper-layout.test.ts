...
describe('Blogger header wrapper layout regression', () => {
  it('flattens only the Header section/widget wrappers so brand, nav, and actions share one flex row', async () => {
    ...
    expect(xml).toContain('.header-bar>.header.section,.header-bar>.header.section>.widget.Header,.header-bar .widget.Header>.widget-content{display:contents}');
    ...
    expect(xml).toContain('html[data-theme=dark] .site-title,html[data-theme=dark] .site-title a');
  });
