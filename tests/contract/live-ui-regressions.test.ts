...
describe('live UI regressions',()=>{
  ...
  it('invokes threadedCommentForm inside threadedComments',async()=>{const s=await readFile('src/widgets/blog-comments.pug','utf8'); const x=s.slice(s.indexOf("b:includable(id='threadedComments'"),s.indexOf("b:includable(id='threadedCommentForm'")); expect(x).toContain("name='threadedCommentForm'");});
